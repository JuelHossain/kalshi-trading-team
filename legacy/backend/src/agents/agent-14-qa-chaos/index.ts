...
export const startSentinel = async (isPaperTrading: boolean) => {
  ...
  try {
    ...
    console.error(
      '[Agent 14] Sentinel Failed to Start (Likely missing or invalid Kalshi Keys)!',
      e
    );
...
export const validateStaleData = (lastUpdated: string | number, maxAgeMinutes: number = 5) => {
  ...
  if (ageMs > maxAgeMs) {
    throw new Error(
      `[Agent 14] STALE DATA VETO: Market data is ${(ageMs / 1000 / 60).toFixed(1)} minutes old (Max: ${maxAgeMinutes}m).`
    );
  }
...
export const validateExecutionerSafety = (isPaperInput: boolean, intendedEnv: 'DEMO' | 'PROD') => {
  ...
  if (intendedEnv === 'DEMO' && !isPaperInput) {
    throw new Error(
      '[Agent 14] CRITICAL SAFETY VIOLATION: Executioner attempting PROD trade in DEMO mode.'
    );
  }

  if (!isPaperInput && intendedEnv !== 'PROD') {
    throw new Error(
      '[Agent 14] CRITICAL SAFETY VIOLATION: Mismatch between env and isPaperTrading flag.'
    );
  }
...
export const auditCodebase = () => {
  ...
  files.forEach((file) => {
    ...
    if (
      ...
      console.warn(
        `[Agent 14] AUDIT WARNING: Potential unbounded array growth in ${path.basename(file)}`
      );
      violations++;
    }
  });
