...
class TestAsyncErrorRace:
    ...
    async def test_fix_critical_errors_logged_synchronously(self):
        ...
        dispatcher = ErrorDispatcher(agent_name="TEST_AGENT", event_bus=event_bus, synapse=synapse)

        # Dispatch CRITICAL error
        await dispatcher.dispatch(
            code="SYSTEM_INIT_FAILED", severity=ErrorSeverity.CRITICAL, domain=ErrorDomain.SYSTEM
        )
    ...
    async def test_multiple_critical_errors_all_logged(self):
        ...
        for code, severity in errors:
            await dispatcher.dispatch(code=code, severity=severity, domain=ErrorDomain.SYSTEM)
