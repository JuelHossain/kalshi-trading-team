...
def hand_agent(mock_vault):
    ...
    agent = HandAgent(agent_id=1, bus=bus, vault=mock_vault, synapse=synapse, kalshi_client=None)
    return agent
...
class TestLiquidityDepthValidation:
    ...
    @pytest.mark.asyncio
    async def test_snipe_check_insufficient_liquidity_best_ask(
        self, hand_agent, mock_kalshi_client
    ):
        """
        ...
        assert not result.get("valid"), "Should reject when liquidity depth < 2x target stake"
        assert (
            "insufficient liquidity depth" in result.get("reason", "").lower()
    ...
    async def test_snipe_check_sufficient_liquidity_best_ask(self, hand_agent, mock_kalshi_client):
                ...
                "asks": [
                    {
                        "price": 50,
                        "count": 500,
                    },  # $250 liquidity at best ask - SUFFICIENT (> 2x $75)
                    {"price": 51, "count": 1000},
                ],
            }
        )
        ...
        assert result.get("valid"), "Should accept when liquidity depth >= 2x target stake"

    @pytest.mark.asyncio
    async def test_snipe_check_insufficient_liquidity_within_spread(
        self, hand_agent, mock_kalshi_client
    ):
        """
        ...
        assert not result.get(
            "valid"
        ), "Should reject when cumulative liquidity within spread < 2x stake"
    ...
    async def test_snipe_check_no_liquidity_data(self, hand_agent, mock_kalshi_client):
        ...
        mock_kalshi_client.get_orderbook = AsyncMock(
            return_value={"bids": [{"price": 45, "count": 1000}], "asks": []}  # No asks available
        )

        result = await hand_agent.snipe_check("TEST-123")

        # Should reject when no liquidity data available
        assert not result.get("valid"), "Should reject when no ask liquidity available"
    ...
    async def test_snipe_check_liquidity_within_spread_only(self, hand_agent, mock_kalshi_client):
        ...
        assert not result.get("valid"), "Should reject when liquidity within spread < 2x stake"
    ...
    async def test_snipe_check_wide_spread_still_rejected(self, hand_agent, mock_kalshi_client):
        ...
        assert not result.get("valid"), "Should reject on wide spread regardless of liquidity"
