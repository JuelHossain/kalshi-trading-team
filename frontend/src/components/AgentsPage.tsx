      ...
      <div className="flex-1 overflow-y-auto pr-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-20">
        {filteredAgents.map((agent) => (
          ...
          >
            ...
            <div
              className={`absolute left-0 top-0 bottom-0 w-1 transition-colors duration-300 ${activeAgentId === agent.id ? 'bg-emerald-500 animate-pulse' : 'bg-transparent group-hover:bg-emerald-500/30'}`}
            ></div>
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3
                    className={`font-bold uppercase tracking-wider text-sm ${viewedAgentId === agent.id ? 'text-white' : 'text-gray-300'}`}
                  >
                    {agent.name}
                  </h3>
