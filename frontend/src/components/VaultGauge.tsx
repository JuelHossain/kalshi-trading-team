import { VaultState } from '@shared/types';
...
const VaultGauge: React.FC<VaultGaugeProps> = ({ compact = false, vault }) => {
  const principal = vault?.principal || 300;
  const currentProfit = vault?.currentProfit || 0;
  const lockThreshold = vault?.lockThreshold || 50;
  const isLocked = vault?.isLocked || currentProfit >= lockThreshold;
  const total = vault?.total || principal + currentProfit;
    ...
    return (
      <div className="w-full">
        ...
        <div className="w-full h-1.5 bg-gray-800/50 rounded-full overflow-hidden flex">
          ...
          ></div>
          <div
            className="h-full bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]"
  ...
  return (
    <div className="bg-white/5 rounded-2xl p-5 border border-white/5 relative overflow-hidden group">
      ...
      <div className="flex items-baseline gap-2 mb-4 relative z-10">
        <span className="text-3xl font-black text-white tracking-tighter font-tech">
          ${total.toFixed(2)}
        </span>
        <span className="text-xs text-emerald-400 font-mono">
          +{((currentProfit / principal) * 100).toFixed(1)}%
        </span>
      </div>
