...
const PnLHeatmap: React.FC = () => {
  ...
  const fetchHeatmapData = async () => {
    ...
    try {
      ...
      // Process RAW data (assuming array of {date, pnl})
      // We need to group by day
      const daysMap = new Map<string, number>(); // DateString -> PnL
      ...
      for (let i = totalDays; i >= 0; i--) {
        ...
        const dayVal = daysMap.get(dateKey);
          ...
          {loading ? (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-[10px] text-emerald-500/50 font-mono animate-pulse">
                  SYNCING CHAIN...
                </span>
              </div>
            </div>
                  ...
                  {filteredData.map((day) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-2 h-10 rounded-full" style={getCellStyle(day)}></div>
                        <div>
                      ...
                      <div
                        className={`font-mono font-bold text-sm ${day.amount >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
                      >
                        {day.amount > 0 ? '+' : ''}
        ...
        <div className="w-[180px] bg-black/20 border-l border-white/5 p-4 flex flex-col shrink-0 backdrop-blur-sm relative z-10">
          {selectedDay ? (
            <div className="animate-[fadeIn_0.3s_ease-out]">
              <div className="mb-4">
                <span className="text-[9px] text-gray-500 font-mono uppercase tracking-widest block mb-1">
                  Selected Date
                </span>
                <div className="text-xl font-bold text-white leading-tight font-tech">
                  {selectedDay.dateObj.getDate()}
                </div>
                <div className="text-xs text-gray-400 font-mono uppercase">
                  {selectedDay.dateObj.toLocaleString('default', {
                    month: 'long',
                    year: 'numeric',
                  })}
                </div>
                <div className="text-[10px] text-gray-600 font-mono">
                  {selectedDay.dateObj.toLocaleString('default', { weekday: 'long' })}
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div
                  className={`text-2xl font-black font-tech tracking-tight ${selectedDay.amount >= 0 ? 'text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.3)]' : 'text-red-400 drop-shadow-[0_0_8px_rgba(248,113,113,0.3)]'}`}
                >
                  {selectedDay.amount > 0 ? '+' : ''}
                  {selectedDay.amount === 0 ? 'IDLE' : `$${selectedDay.amount}`}
                </div>
              </div>

              <div className="flex-1 overflow-y-auto no-scrollbar">
                <span className="text-[9px] text-gray-500 font-mono uppercase tracking-widest block mb-2">
                  AI Reasoning
                </span>
