...
const SystemHealth: React.FC<SystemHealthProps> = ({ onClose }) => {
    const [testResults, setTestResults] = useState<Record<string, { status: 'PENDING' | 'PASS' | 'FAIL', latency: number, msg: string }>>({
    ...
    const runTests = async () => {
        ...
        try {
            ...
            update('GEMINI (Analyst)', 'PASS', Math.round(performance.now() - startGemini), 'Ready to Debate');
        } catch (e: any) {
        ...
        try {
            ...
            update('GROQ (Scout)', 'PASS', Math.round(performance.now() - startGroq), 'Llama-3.3-70b-versatile Active');
        } catch (e: any) {
        ...
        try {
            ...
            update('KALSHI V2 (Exchange)', 'PASS', Math.round(performance.now() - startKalshi), 'Authenticated (RSA-SHA256)');
        } catch (e: any) {
        ...
        try {
            ...
            update('RAPIDAPI (Odds)', 'PASS', Math.round(performance.now() - startRapid), 'Odds-Vegas Endpoint Reachable');
        } catch (e: any) {
        ...
        try {
            ...
            update('SUPABASE (DB)', 'PASS', Math.round(performance.now() - startSupabase), 'PostgreSQL Connection Stable');
        } catch (e: any) {
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-[#050505] border border-emerald-500/30 rounded-2xl w-full max-w-2xl p-6 shadow-[0_0_50px_rgba(16,185,129,0.2)]">
                        ...
                        return (
                            <div key={name} className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/5">
                                <div className="flex items-center gap-4">
                                    <div className={`w-3 h-3 rounded-full ${data.status === 'PASS' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' :
                                        data.status === 'FAIL' ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' :
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    {data.status !== 'PENDING' && (
                                        <span className={`font-mono text-xs px-2 py-1 rounded bg-black/50 ${data.status === 'PASS' ? 'text-emerald-500' : 'text-red-500'}`}>
                                            {data.latency}ms
                                        </span>
                                    )}
                                </div>
                ...
                <div className="mt-6 flex justify-end gap-3">
                    <button onClick={runTests} className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 text-xs font-bold rounded-lg uppercase tracking-wider transition-all">
                        Rerun Diagnostics
                    </button>
                    <button onClick={onClose} className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-xs font-bold rounded-lg uppercase tracking-wider transition-all">
                        Close Panel
                    </button>
                </div>
            </div>
