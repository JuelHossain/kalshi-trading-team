...
export const auditDecision = async (
  ...
  try {
    const systemPrompt =
      'You are the Pessimist Auditor. Your job is to find reasons why a trade will FAIL. Be cynical and paranoid. Return STRICTLY valid JSON.';
    ...
    return {
      approved: false,
      reason: `Audit Error: ${e.message} - Fail Closed`,
      auditorConfidence: 0,
    };
  }
};
