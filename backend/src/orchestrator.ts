...
export const runOrchestratorCycle = async (
    ...
    const addLog = (message: string, agentId: number, phaseId: number, level: 'INFO' | 'WARN' | 'ERROR' | 'SUCCESS' = 'INFO') => {
        const log = {
    ...
    const completeCycle = () => {
        ...
        addLog(`✓ CYCLE #${cycleCount} COMPLETE - All phases finished successfully`, 0, currentPhase, 'SUCCESS');
    };
    ...
    try {
        ...
        addLog(`[GHOST] CYCLE START: Sentient Alpha Funnel Protocol (Cycle #${cycleCount})`, 1, 0, 'INFO');
        ...
        addLog(`Analyst [Gemini Pro]: ${analystReport.judgeVerdict} (Conf: ${analystReport.confidenceScore}%)`, 4, 2, 'SUCCESS');
        ...
        addLog(`Simulation Scientist: EV ${simResults.ev.toFixed(3)}, Status: ${simResults.status}`, 5, 2, simResults.status === 'WARNING' ? 'WARN' : 'INFO');
        ...
        if (wager > 0) {
            ...
            addLog(`SNIPE EXECUTED: Order ID ${order?.order_id || 'PENDING'} @ ${sniperAnalysis.snipePrice}c`, 8, 3, 'SUCCESS');
