...
export const fetchPortfolioBalance = async (isPaperTrading: boolean) => {
    ...
    try {
        ...
        if (totalSpend > BUDGET_CEILING) {
            console.error(`[Agent 9] BUDGET CEILING BREACHED: $${totalSpend.toFixed(2)}. Freezing execution.`);
        ...
        if (dailyProfit >= PROFIT_THRESHOLD) {
            console.log(`[Agent 9] RECURSIVE VAULT ACTIVE: Profit ($${(dailyProfit / 100).toFixed(2)}) > $50. House Money only.`);
        ...
        console.log(`[Agent 9] Auditor Report: Balance: $${(balanceCents / 100).toFixed(2)} | Spend: $${totalSpend.toFixed(4)} | Avail: $${(availableCents / 100).toFixed(2)}`);
