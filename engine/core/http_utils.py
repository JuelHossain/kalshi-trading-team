...
def unauthorized_response(message: str = "Valid API key required") -> web.Response:
    ...
    return web.json_response({"error": "Unauthorized", "message": message}, status=401)


def success_response(message: str, **kwargs) -> web.Response:
    """Create a standardized success response."""
    return web.json_response({"success": True, "message": message, **kwargs})


def auth_response(
    is_authenticated: bool, mode: str, is_production: bool, message: str = ""
) -> web.Response:
