...
class RecursiveVault:
    ...
    async def initialize(self, current_balance_cents: int):
        async with self._lock:
            ...
            print(f"{Fore.CYAN}[VAULT] Initialized. SOD Balance: ${self.start_of_day_balance/100:.2f}{Style.RESET_ALL}")
    ...
    async def _check_kill_switch(self):
        ...
        if self.current_balance < threshold:
            if not self.kill_switch_active:
                print(f"{Fore.RED}[VAULT] 🚨 CRITICAL: Balance (${self.current_balance/100:.2f}) < 85% Principal Threshold (${threshold/100:.2f}).{Style.RESET_ALL}")
                print(f"{Fore.RED}[VAULT] KILL SWITCH ACTIVATED. HALTING ALL OPERATIONS.{Style.RESET_ALL}")
    ...
    async def _check_lock(self) -> bool:
        ...
        if daily_profit >= self.DAILY_PROFIT_THRESHOLD_CENTS:
            if not self.is_locked:
                print(f"{Fore.GREEN}[VAULT] 🔒 PROFIT THRESHOLD (${daily_profit/100:.2f}) REACHED.{Style.RESET_ALL}")
                print(f"{Fore.GREEN}[VAULT] PRINCIPAL PROTECTION ACTIVATED. FROZEN ${self.PRINCIPAL_CAPITAL_CENTS/100:.2f}.{Style.RESET_ALL}")
                self.is_locked = True
            return True
